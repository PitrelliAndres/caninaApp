"""
Rutas de onboarding de nuevos usuarios
"""
from flask import Blueprint, request, jsonify, current_app
from app import db
from app.models import User, Dog, UserPreference, UserRole
from app.utils.auth import login_required
from app.utils.validators import validate_nickname, validate_age, validate_dog_age
from datetime import datetime

onboarding_bp = Blueprint('onboarding', __name__)

# Almacenamiento temporal de datos de onboarding
onboarding_data = {}

@onboarding_bp.route('/check-nickname', methods=['POST'])
@login_required
def check_nickname():
    """Verificar disponibilidad de nickname"""
    try:
        data = request.get_json()
        nickname = data.get('nickname', '').strip()
        
        if not validate_nickname(nickname):
            return jsonify({
                'available': False,
                'message': 'Nickname must be 3-20 alphanumeric characters'
            }), 200
        
        # Verificar si ya existe
        existing = User.query.filter_by(nickname=nickname).first()
        if existing and existing.id != request.current_user_id:
            return jsonify({
                'available': False,
                'message': 'Nickname already taken'
            }), 200
        
        return jsonify({
            'available': True,
            'message': 'Nickname available'
        }), 200
        
    except Exception as e:
        current_app.logger.error(f"Check nickname error: {str(e)}")
        return jsonify({'error': 'Internal server error'}), 500

@onboarding_bp.route('/step1', methods=['POST'])
@login_required
def step1_user_profile():
    """Paso 1: Perfil de usuario"""
    try:
        user = User.query.get(request.current_user_id)
        if not user:
            return jsonify({'error': 'User not found'}), 404
        
        data = request.get_json()
        
        # Validaciones
        nickname = data.get('nickname', '').strip()
        if not validate_nickname(nickname):
            return jsonify({'error': 'Invalid nickname format'}), 400
        
        # Verificar unicidad
        existing = User.query.filter_by(nickname=nickname).first()
        if existing and existing.id != user.id:
            return jsonify({'error': 'Nickname already taken'}), 400
        
        age = data.get('age')
        if not validate_age(age):
            return jsonify({'error': 'Age must be between 10 and 99'}), 400
        
        # Guardar en memoria temporal
        if user.id not in onboarding_data:
            onboarding_data[user.id] = {}
        
        onboarding_data[user.id]['step1'] = {
            'nickname': nickname,
            'age': int(age),
            'avatar': data.get('avatar')
        }
        
        return jsonify({'success': True}), 200
        
    except Exception as e:
        current_app.logger.error(f"Onboarding step 1 error: {str(e)}")
        return jsonify({'error': 'Internal server error'}), 500

@onboarding_bp.route('/step2', methods=['POST'])
@login_required
def step2_dog_profile():
    """Paso 2: Perfil de mascota"""
    try:
        user_id = request.current_user_id
        
        # Verificar que pasó por step 1
        if user_id not in onboarding_data or 'step1' not in onboarding_data[user_id]:
            return jsonify({'error': 'Must complete step 1 first'}), 400
        
        data = request.get_json()
        dog_data = data.get('dog', {})
        
        # Validaciones
        name = dog_data.get('name', '').strip()
        if not name:
            return jsonify({'error': 'Dog name is required'}), 400
        
        age = dog_data.get('age')
        if not validate_dog_age(age):
            return jsonify({'error': 'Dog age must be between 0 and 25'}), 400
        
        breed = dog_data.get('breed_custom') or 'Unknown'
        
        # Guardar en memoria temporal
        onboarding_data[user_id]['step2'] = {
            'dog': {
                'name': name,
                'age': int(age),
                'breed': breed,
                'photo_url': dog_data.get('avatar')
            }
        }
        
        return jsonify({'success': True}), 200
        
    except Exception as e:
        current_app.logger.error(f"Onboarding step 2 error: {str(e)}")
        return jsonify({'error': 'Internal server error'}), 500

@onboarding_bp.route('/step3', methods=['POST'])
@login_required
def step3_preferences():
    """Paso 3: Preferencias y privacidad - Guardar todo"""
    try:
        user_id = request.current_user_id
        
        # Verificar que pasó por steps anteriores
        if (user_id not in onboarding_data or 
            'step1' not in onboarding_data[user_id] or 
            'step2' not in onboarding_data[user_id]):
            return jsonify({'error': 'Must complete previous steps first'}), 400
        
        user = User.query.get(user_id)
        if not user:
            return jsonify({'error': 'User not found'}), 404
        
        data = request.get_json()
        privacy = data.get('privacy', {})
        interests = data.get('interests', [])
        
        # Iniciar transacción
        try:
            # 1. Actualizar usuario con datos del step 1
            step1_data = onboarding_data[user_id]['step1']
            user.nickname = step1_data['nickname']
            user.age = step1_data['age']
            if step1_data.get('avatar'):
                user.avatar_url = step1_data['avatar']
            
            # 2. Actualizar configuración de privacidad
            user.is_public = privacy.get('public_profile', True)
            user.allow_matching = privacy.get('enable_match', True)
            user.allow_proximity = privacy.get('enable_proximity', False)
            
            # 3. Crear perro
            step2_data = onboarding_data[user_id]['step2']['dog']
            dog = Dog(
                user_id=user_id,
                name=step2_data['name'],
                age=step2_data['age'],
                breed=step2_data['breed'],
                photo_url=step2_data.get('photo_url'),
                is_vaccinated=True,
                created_at=datetime.utcnow()
            )
            db.session.add(dog)
            
            # 4. Crear preferencias
            preferences = UserPreference(
                user_id=user_id,
                interests=interests,
                preferred_age_min=max(user.age - 10, 18) if user.age else 18,
                preferred_age_max=min(user.age + 10, 100) if user.age else 100
            )
            db.session.add(preferences)
            
            # Commit todo
            db.session.commit()
            
            # Limpiar datos temporales
            del onboarding_data[user_id]
            
            return jsonify({
                'success': True,
                'redirect': '/'
            }), 200
            
        except Exception as e:
            db.session.rollback()
            raise e
        
    except Exception as e:
        current_app.logger.error(f"Onboarding step 3 error: {str(e)}")
        return jsonify({'error': 'Internal server error'}), 500
