...
class OptimizedMobileSocketClient {
  ...
  async connect(token, options = {}) {
    ...
    try {
      ...
      // TODO: harden for production - use environment variable or config
      const backendUrl = process.env.WS_URL || 'http://localhost:5000'
  ...
  disconnect() {
    ...
    // Limpiar listeners móviles (Pure React Native)
